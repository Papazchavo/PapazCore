    ...
    public void onChat(AsyncPlayerChatEvent event) {
        ...
        if (!player.hasPermission("papaz.admin")) {
            ...
            if (lowerMessage.contains(".com") || lowerMessage.contains(".net") || 
                ...
                player.sendMessage(plugin.getPrefix() + PapazCore.colorize("&cReklam yapmak yasaktir!"));
                return;
            }
        }
        event.setCancelled(true);
        String format;
        if (player.hasPermission("papaz.admin")) {
            format = "&8[&c&lADMIN&8] &c%s &8>> &f%s";
        } else if (player.hasPermission("papaz.mod")) {
            format = "&8[&9MOD&8] &9%s &8>> &f%s";
        } else if (player.hasPermission("papaz.vip")) {
            format = "&8[&6VIP&8] &e%s &8>> &f%s";
        } else {
            format = "&8[&7Uye&8] &7%s &8>> &f%s";
        }
